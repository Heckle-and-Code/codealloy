...
exports.AgentChatViewProvider = void 0;
...
const shadowGitService_1 = require("./shadowGitService");
const inlineDiffService_1 = require("./inlineDiffService");
const mcpService_1 = require("./mcpService");
class AgentChatViewProvider {
    _extensionUri;
    _modelManager;
    _llamaServer;
    static viewType = 'codealloy.agentView';
    _view;
    _messages = [];
    _activeRequest;
    _currentPrompt;
    _currentAssistantMsg;
    _autonomyLevel = 'L3'; // L0, L1, L2 (Supervisor), L3 (Guarded), L4 (Autonomous)
    _outputChannel;
    _onDidChangeAutonomyLevel = new vscode.EventEmitter();
    onDidChangeAutonomyLevel = this._onDidChangeAutonomyLevel.event;
    _pendingApprovals = new Map();
    _shadowGit;
    _diffService;
    _mcpService;
    constructor(_extensionUri, _modelManager, _llamaServer) {
        this._extensionUri = _extensionUri;
        this._modelManager = _modelManager;
        this._llamaServer = _llamaServer;
        this._outputChannel = vscode.window.createOutputChannel('CodeAlloy Agent Chat');
    }
    getShadowGit() {
        if (!this._shadowGit) {
            const workspacePath = this._getWorkspacePath();
            this._shadowGit = new shadowGitService_1.ShadowGitService(workspacePath, this._outputChannel);
        }
        return this._shadowGit;
    }
    getDiffService() {
        if (!this._diffService) {
            this._diffService = new inlineDiffService_1.InlineDiffService();
        }
    ...
    getMcpService() {
        if (!this._mcpService) {
            const workspacePath = this._getWorkspacePath();
            this._mcpService = new mcpService_1.McpService(workspacePath, this._outputChannel);
        }
    ...
    _isDangerousCommand(command) {
        return (/\brm\s+-rf\s+[/~]/i.test(command) ||
    ...
    async _requestUserApproval(type, target, details, assistantMsgId) {
    ...
    async _executeFileAction(fileName, content, assistantMsgId) {
        try {
    ...
    async _executeShellCommand(command, assistantMsgId) {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ||
        ...
        if (this._isDangerousCommand(command)) {
            ...
            const choice = await vscode.window.showWarningMessage(`CodeAlloy Security Guard:\n\nThe agent proposed executing a sensitive or destructive command:\n"${command}"\n\nDo you want to allow this command to run?`, { modal: true }, 'Approve Command', 'Block Command');
        ...
        return new Promise((resolve) => {
            (0, child_process_1.exec)(command, { cwd: workspaceFolder, maxBuffer: 10 * 1024 * 1024, timeout: 60000 }, (err, stdout, stderr) => {
                if (err) {
    ...
    async _detectAndExecuteFiles(prompt, content, assistantMsgId) {
        ...
        const hasActionIntent = fileIntentRegex.test(prompt) ||
    ...
    async _executeInferenceStream(provider, externalEndpoint, activeModel, messages, tools, assistantMsgId, onChunk) {
        ...
        if (provider === 'embedded') {
            ...
            try {
                const parsed = new url_1.URL(externalEndpoint);
    ...
    async _handleUserPrompt(prompt, incomingTurnId) {
        ...
        while (turnCount < MAX_AGENTIC_TURNS) {
            ...
            let turnContent = '';
            const streamSuccess = await this._executeInferenceStream(provider, externalEndpoint, activeModel, conversationMessages, tools, assistantMsgId, (chunk) => {
                turnContent += chunk;
...
exports.AgentChatViewProvider = AgentChatViewProvider;
