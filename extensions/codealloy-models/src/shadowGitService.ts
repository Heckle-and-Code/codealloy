...
export class ShadowGitService {
	...
	private readonly _projectHash: string;
	private readonly _projectDir: string;
	private readonly _gitDir: string;
	private readonly _indexFile: string;
	private readonly _outputChannel: vscode.OutputChannel;
	private _initialized: boolean = false;
	private _historyTimeline: TimelineEntry[] = [];
	 ...
	 */
	public async createPreTurnSnapshot(turnId: string, prompt: string): Promise<string | null> {
		await this.init();
		const t0 = Date.now();
		...
		try {
			...
			const commitHash = (
				await this._execGitWithWorkTree(
					`commit-tree ${treeHash} -m "Pre-turn checkpoint [${turnId}]: ${sanitizedPrompt}"`
				)
			).trim();
	...
	public async finalizeTurn(
		...
		try {
			...
			const diffOutput = (
				await this._execGit(`--git-dir="${this._gitDir}" diff-tree --no-commit-id --name-only -r ${preCommitHash} ${postTree}`)
			).trim();
			...
			this._outputChannel.appendLine(
				`[ShadowGit] Finalized turn ${turnId}. Files modified: ${modifiedFiles.length > 0 ? modifiedFiles.join(', ') : 'none'}`
			);
	 ...
	 */
	public async rollbackToCommit(commitHash: string): Promise<boolean> {
		await this.init();
		const t0 = Date.now();
	 ...
	 */
	public async undoLastTurn(): Promise<{ success: boolean; entry?: TimelineEntry }> {
	 ...
	 */
	public getHistoryTimeline(): TimelineEntry[] {
		return [...this._historyTimeline];
	}

	private _saveTimeline(): void {
		try {
	...
	private _execGit(commandArgs: string): Promise<string> {
		return new Promise((resolve, reject) => {
			exec(`git ${commandArgs}`, { maxBuffer: 20 * 1024 * 1024 }, (err, stdout, stderr) => {
				if (err) {
	...
	private _execGitWithWorkTree(commandArgs: string): Promise<string> {
		const env = {
			...process.env,
