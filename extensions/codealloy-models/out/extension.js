...
const llamaServerService_1 = require("./llamaServerService");
const agentChatViewProvider_1 = require("./agentChatViewProvider");
const hardwareProber_1 = require("./hardwareProber");
const ollamaService_1 = require("./ollamaService");
let modelStatusBarItem;
let chatStatusBarItem;
let autonomyStatusBarItem;
let modelManager;
let llamaServer;
let chatProvider;
function activate(context) {
    modelManager = new modelManager_1.LocalModelManager();
    llamaServer = llamaServerService_1.LlamaServerService.getInstance();
    const hw = hardwareProber_1.HardwareProber.getHardwareInfo();
    ...
    modelStatusBarItem = vscode.window.createStatusBarItem('codealloy.modelSelector', vscode.StatusBarAlignment.Right, 100);
    modelStatusBarItem.command = 'codealloy.selectModel';
    context.subscriptions.push(modelStatusBarItem);
    autonomyStatusBarItem = vscode.window.createStatusBarItem('codealloy.autonomySelector', vscode.StatusBarAlignment.Right, 99);
    autonomyStatusBarItem.command = 'codealloy.selectAutonomy';
    context.subscriptions.push(autonomyStatusBarItem);
    chatStatusBarItem = vscode.window.createStatusBarItem('codealloy.chatButton', vscode.StatusBarAlignment.Right, 101);
    ...
    chatProvider = new agentChatViewProvider_1.AgentChatViewProvider(context.extensionUri, modelManager, llamaServer);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(agentChatViewProvider_1.AgentChatViewProvider.viewType, chatProvider, { webviewOptions: { retainContextWhenHidden: true } }));
    chatProvider.onDidChangeAutonomyLevel((level) => {
        updateAutonomyStatusBar(level);
    });
    // 3. Register Commands
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.selectAutonomy', async () => {
        await showAutonomyPicker();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.undoLastTurn', async () => {
        const shadowGit = chatProvider.getShadowGit();
        const result = await shadowGit.undoLastTurn();
        if (result.success && result.entry) {
            vscode.window.showInformationMessage(`CodeAlloy: Successfully rolled back last turn (${result.entry.commitHash.substring(0, 8)}).`);
        }
        else {
            vscode.window.showInformationMessage('CodeAlloy: No agent turns available to undo.');
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.timeTravelTimeline', async () => {
        await showTimeTravelQuickPick();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.acceptDiffChunk', async () => {
        await chatProvider.getDiffService().acceptCurrentChunk();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.rejectDiffChunk', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.switchProvider', async () => {
        await switchInferenceProvider();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.refreshModels', () => {
        updateStatusBar();
        chatProvider.syncState();
        vscode.window.showInformationMessage('CodeAlloy: Local models refreshed.');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.clearChat', () => {
        chatProvider.clearChat();
        vscode.window.showInformationMessage('CodeAlloy: Agent chat history cleared.');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.focusChat', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.addLocalModel', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codealloy.downloadModel', async (model) => {
        if (!model) {
            const choice = await vscode.window.showQuickPick(modelCatalog_1.CURATED_MODELS.map(m => ({
                label: m.displayName,
...
async function switchInferenceProvider() {
    const config = vscode.workspace.getConfiguration('codealloy');
    const currentProvider = config.get('inferenceProvider', 'embedded');
    const currentEndpoint = config.get('externalEndpoint', 'http://127.0.0.1:11434');
    ...
    if (choice.provider === 'external') {
        ...
        const discovery = await ollamaService_1.OllamaService.discoverModels(newEndpoint.trim());
...
async function activateModel(fileName, curatedInfo) {
    const config = vscode.workspace.getConfiguration('codealloy');
    const provider = config.get('inferenceProvider', 'embedded');
    ...
    if (provider === 'embedded') {
        ...
        const matchedCurated = curatedInfo || matched?.curatedInfo || modelCatalog_1.CURATED_MODELS.find(c => c.fileName.toLowerCase() === fileName.toLowerCase());
        const safety = hardwareProber_1.HardwareProber.checkMemorySafety(matched ? matched.name : fileName, matchedCurated?.recommendedRamGb, matched?.sizeBytes || matchedCurated?.fileSizeBytes);
        if (!safety.safe && safety.warning) {
            const choice = await vscode.window.showWarningMessage(safety.warning, { modal: true }, 'Load Model Anyway');
...
function updateStatusBar() {
    ...
    const config = vscode.workspace.getConfiguration('codealloy');
    const provider = config.get('inferenceProvider', 'embedded');
    const externalEndpoint = config.get('externalEndpoint', 'http://127.0.0.1:11434');
    const hw = hardwareProber_1.HardwareProber.getHardwareInfo();
    if (active) {
...
function updateAutonomyStatusBar(level) {
...
async function showAutonomyPicker() {
...
async function showModelPicker() {
    ...
    if (provider === 'external') {
        ...
        try {
            const discovery = await ollamaService_1.OllamaService.discoverModels(externalEndpoint);
            if (discovery.available && discovery.models.length > 0) {
    ...
    const notInstalled = modelCatalog_1.CURATED_MODELS.filter(c => !installed.some(i => i.fileName.toLowerCase() === c.fileName.toLowerCase()));
...
async function downloadModelWithProgress(model) {
    await vscode.window.withProgress({
        ...
        try {
            ...
            vscode.window.showInformationMessage(`CodeAlloy: "${model.displayName}" downloaded and activated in GPU memory!`, 'OK');
        }
        catch (err) {
...
async function showTimeTravelQuickPick() {
    const shadowGit = chatProvider.getShadowGit();
